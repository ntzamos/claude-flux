...
-- Unique constraint to prevent duplicate rules on repeated migration runs
ALTER TABLE grading_rulebook ADD COLUMN IF NOT EXISTS rule_hash TEXT;
UPDATE grading_rulebook SET rule_hash = MD5(category || '|' || rule) WHERE rule_hash IS NULL;
CREATE UNIQUE INDEX IF NOT EXISTS grading_rulebook_unique_rule ON grading_rulebook (MD5(category || '|' || rule));
